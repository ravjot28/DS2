...
<BODY>
	<FONT FACE=\ "Arial\" COLOR=\"#0000FF\">
 	...
 	String ccexpmon = request.getParameter("ccexpmon");
 	if (ccexpmon == null) {
 		ccexpmon = "";
 	}
 ...
 %>
 	...
 	String[] countries = { "United States", "Australia", "Canada", "Chile", "China", "France",
 						"Germany", "Japan", "Russia", "South Africa", "UK" };
 ...
 %>
		<FORM ACTION='./dsnewcustomer.jsp' METHOD=GET>
			Firstname <INPUT TYPE=TEXT NAME='firstname' VALUE='<%=firstname%>'
				SIZE=16 MAXLENGTH=50>* <BR> Lastname <INPUT TYPE=TEXT
				NAME='lastname' VALUE='<%=lastname%>' SIZE=16 MAXLENGTH=50>*
			<BR> Address1 <INPUT TYPE=TEXT NAME='address1'
				VALUE='<%=address1%>' SIZE=16 MAXLENGTH=50>* <BR>
			Address2 <INPUT TYPE=TEXT NAME='address2' VALUE='<%=address2%>'
				SIZE=16 MAXLENGTH=50> <BR> City <INPUT TYPE=TEXT
				NAME='city' VALUE='<%=city%>' SIZE=16 MAXLENGTH=50>* <BR>
			State <INPUT TYPE=TEXT NAME='state' VALUE='<%=state%>' SIZE=16
				MAXLENGTH=50> <BR> Zipcode <INPUT TYPE=TEXT NAME='zip'
				VALUE='<%=zip%>' SIZE=16 MAXLENGTH=5> <BR> Country <SELECT
				NAME='country' SIZE=1>
				<%
				...
				%>
				<OPTION VALUE="<%=countries[i]%>" SELECTED>
					<%=countries[i]%></OPTION>
				<%
					} else {
				%>
				...
				%>
			</SELECT>* <BR> Email <INPUT TYPE=TEXT NAME='email' VALUE='<%=email%>'
				SIZE=16 MAXLENGTH=50> <BR> Phone <INPUT TYPE=TEXT
				NAME='phone' VALUE='<%=phone%>' SIZE=16 MAXLENGTH=50> <BR>
			Credit Card Type <SELECT NAME='creditcardtype' SIZE=1>
				<%
					int j = 0;
				...
				%>
			</SELECT> Credit Card Number <INPUT TYPE=TEXT NAME='creditcard'
				VALUE='<%=creditcard%>' SIZE=16 MAXLENGTH=50> Credit Card
			Expiration <SELECT NAME='ccexpmon' SIZE=1>
				<%
				...
				%>
				<OPTION VALUE="<%=j%>"><%=months[i]%>
				</OPTION>
				<%
					}
								}
				%>
			</SELECT> <SELECT NAME='ccexpyr' SIZE=1>
				<%
					int yr;
				...
				VALUE='<%=username%>' SIZE=16 MAXLENGTH=50>* <BR>
			Password <INPUT TYPE='PASSWORD' NAME='password' VALUE='<%=password%>'
				SIZE=16 MAXLENGTH=50>* <BR> Age <INPUT TYPE=TEXT
				NAME='age' VALUE='<%=age%>' SIZE=3 MAXLENGTH=3> <BR>
			Income (\$US) <INPUT TYPE=TEXT NAME='income' VALUE='<%=income%>'
				SIZE=16 MAXLENGTH=50> <BR> Gender <INPUT TYPE=RADIO
				NAME='gender' VALUE="M" <%if (gender == "M") {%> CHECKED <%}%>>
			Male <INPUT TYPE=RADIO NAME='gender' VALUE="F"
				<%if (gender == "F") {%> CHECKED <%}%>> Female <INPUT
				TYPE=RADIO NAME='gender' VALUE="?" <%if (gender == "?") {%> CHECKED
				<%}%>> Don't Know <BR> <INPUT TYPE='submit'
				VALUE='Submit New Customer Data'>
		</FORM> <%
 	} else // executed if the username requested did not exist - insert the new user
 			{
 				...
 				String insert_newuser_query = "INSERT INTO DS2.CUSTOMERS (FIRSTNAME, LASTNAME, ADDRESS1, ADDRESS2, "
 						...
 						+ " USERNAME, PASSWORD, AGE, INCOME, GENDER) " + " VALUES ('" + firstname + "','"
 						+ lastname + " ','" + address1 + " ','" + address2 + "','" + city + "','" + state
 						+ "','" + zip + "','" + country + "','" + region + "','" + email + "','" + phone + "','"
 						+ creditcardtype + "','" + creditcard + "','" + creditcardexpiration + "','" + username
 						+ "','" + password + "','" + age + "','" + income + "','" + gender + "');";
			...
			<H2>
				<FORM ACTION='./dsbrowse.jsp' METHOD=GET>
					<INPUT TYPE=HIDDEN NAME=customerid VALUE=<%=customerid%>> <INPUT
						TYPE=SUBMIT VALUE='Start Shopping'>
				</FORM>
				<%
					} catch (Exception e) {
					...
					{
				%>
				<H2>New Customer - Please Complete All Required Fields Below
					(marked with *)hammam</H2>

				<%
					String[] countries = { "United States", "Australia", "Canada", "Chile", "China", "France", "Germany",
								"Japan", "Russia", "South Africa", "UK" };
						String[] cctypes = { "MasterCard", "Visa", "Discover", "Amex", "Dell Preferred" };
						String[] months = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov",
								"Dec" };
				%>
				<FORM ACTION='./dsnewcustomer.jsp' METHOD=GET>
					...
					Address2 <INPUT TYPE=TEXT NAME='address2' VALUE='<%=address2%>'
						...
						NAME='city' VALUE='<%=city%>' SIZE=16 MAXLENGTH=50>* <BR>
					State <INPUT TYPE=TEXT NAME='state' VALUE='<%=state%>' SIZE=16
						MAXLENGTH=50> <BR> Zipcode <INPUT TYPE=TEXT
						NAME='zip' VALUE='<%=zip%>' SIZE=16 MAXLENGTH=5> <BR>
					...
					Password <INPUT TYPE='PASSWORD' NAME='password'
						VALUE='<%=password%>' SIZE=16 MAXLENGTH=50>* <BR> Age
					<INPUT TYPE=TEXT NAME='age' VALUE='<%=age%>' SIZE=3 MAXLENGTH=3>
					<BR> Income (US) <INPUT TYPE=TEXT NAME='income'
						VALUE='<%=income%>' SIZE=16 MAXLENGTH=50> <BR> Gender
					<INPUT TYPE=RADIO NAME='gender' VALUE="M" <%if (gender == "M") {%>
						CHECKED <%}%>> Male <INPUT TYPE=RADIO NAME='gender'
						VALUE="F" <%if (gender == "F") {%> CHECKED <%}%>> Female <INPUT
						TYPE=RADIO NAME='gender' VALUE="?" <%if (gender == "?") {%>
						CHECKED <%}%>> Don't Know <BR> <INPUT TYPE='submit'
						VALUE='Submit New Customer Data'>
				</FORM>
				<%
					}
				...
				<HR>
				<P ALIGN=CENTER>
					Thank You for Visiting the DVD Store!</A>
				</P>
				<HR>
