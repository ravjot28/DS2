...
<HTML>
<HEAD>
<TITLE>DVD Store Browse Page</TITLE>
</HEAD>
<BODY>
	<FONT FACE=\ "Arial\" COLOR=\"#0000FF\">
 	...
 	{
 %>
		<H2>You have not logged in - Please click below to Login to DVD
			Store</H2>
 ...
 %>
		...
		<FORM ACTION='./dsbrowse.jsp' METHOD=GET>
			...
			<INPUT NAME='browsetype' TYPE=RADIO VALUE='title'
				<%if (browsetype == "title") {%> CHECKED <%}%>>Title <INPUT
				NAME='browse_title' VALUE='<%=browse_title%>' TYPE=TEXT SIZE=15>
			<BR> <INPUT NAME='browsetype' TYPE=RADIO VALUE='actor'
			...
			<BR> <INPUT NAME='browsetype' TYPE=RADIO VALUE='category'
				...
				%>
			</SELECT><BR> Number of search results to return <SELECT NAME='limit_num'>
				<%
				...
				%>
			</SELECT><BR> <INPUT TYPE=HIDDEN NAME='customerid' VALUE=<%=customerid%>>
			<%
 	...
 	if (browsetype != null) // if a browse type of either TITLE AUTHOR or CATEGORY was selected
 	{
 		...
 		try {
 			...
 			{
 			case 't':
 				browse_query = "select * from PRODUCTS where MATCH (TITLE) AGAINST ('" + browse_title
 						+ "') LIMIT " + limit_num + ";";
 				break;
 			case 'a':
 				browse_query = "select * from PRODUCTS where MATCH (ACTOR) AGAINST ('" + browse_actor
 						+ "') LIMIT " + limit_num + ";";
 				break;
 			case 'c':
 				browse_query = "select * from PRODUCTS where CATEGORY = " + browse_category
 						+ " and SPECIAL=1 LIMIT " + limit_num + ";";
 				break;
 			}
 ...
 %> <BR>
		...
		<FORM ACTION='./dsbrowse.jsp' METHOD=GET>
			<TABLE border=2>
				...
				<%
					String browse_result_row_prod_id, browse_result_row_title, browse_result_row_actor,
										browse_result_row_price;
								do {
				...
				%>
				<TR>
					<TD><INPUT NAME='selected_item' TYPE=CHECKBOX
						VALUE=<%=browse_result_row_prod_id%>></TD>
