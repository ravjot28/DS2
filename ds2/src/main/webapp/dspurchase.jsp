...
<HTML>
<HEAD>
<TITLE>DVD Store Purchase Page</TITLE>
</HEAD>
<BODY>
 ...
 %>
		<H2>Selected Items: specify quantity desired; click Purchase when
			finished</H2> <BR>
		<FORM ACTION='./dspurchase.jsp' METHOD=GET>
			<TABLE border=2>
				...
				<TR>
					...
					<TD><INPUT NAME='quan' TYPE='TEXT' SIZE=10
						VALUE='<%=quan[i]%>'></TD>
					<TD><%=title%></TD>
					<TD><%=actor%></TD>
					<TD ALIGN=RIGHT><%=amt%></TD>
					<TD ALIGN=CENTER><INPUT NAME='drop' TYPE=CHECKBOX
						VALUE='<%=item[i]%>'></TD>
				</TR>
				<%
		...
		<FORM ACTION='./dspurchase.jsp' METHOD=GET>
			<INPUT TYPE=HIDDEN NAME='confirmpurchase' VALUE='yes'> <INPUT
				...
				for (int i = 0; i < item_length; i++) // loop for building item and quan list for a purchase
					{
						...
						{
			%>
			<INPUT NAME='item' TYPE='HIDDEN' VALUE='<%=item[i]%>'> <INPUT
				NAME='quan' TYPE='HIDDEN' VALUE='<%=quan[i]%>'>
			<%
				}
					}
			%>
					...
					try {
						...
						try {
							...
							purchase_insert_query = "INSERT into DS2.ORDERS (ORDERDATE, CUSTOMERID, NETAMOUNT, TAX, TOTALAMOUNT)"
									+ " VALUES ( '" + currentdate + "'," + customerid + "," + totals.format(netamount) + ","
									+ totals.format(taxamount) + "," + totals.format(totalamount) + ");";
							...
							while (h < item.length) {
								...
								{
			%>
			Insufficient quantity for prod
			<%=item[h]%>
			<%
				success = false;
								} else // if quantity does exist, update to new level
								{
									query = "UPDATE DS2.INVENTORY SET QUAN_IN_STOCK=" + new_quan + ", SALES=" + new_sales
											+ " WHERE PROD_ID=" + item[h] + ";";
									purchaseupdateStatement.executeUpdate(query);
								}

								p_query = p_query + "(" + j + "," + orderid + "," + item[h] + "," + quan[h] + ",'"
										+ currentdate + "'),";
								if (h < 10) {
			...
			<H3>
				...
				<%=creditcard%>
				(
				<%=cctypes[creditcardtype - 1]%>), expiration
				<%=creditcardexpiration%></H3>
			<BR>
			<H2>
				Order Completed Successfully --- ORDER NUMBER:
				<%=orderid%></H2>
			<BR>
			<%
				} catch (Exception e) {
