...
<HTML>
<HEAD>
<TITLE>DVD Store Login Page</TITLE>
</HEAD>
<BODY>
 ...
 %>
 	...
 	query = "select PROD_ID FROM DS2.CUST_HIST where CUSTOMERID=" + customerid
					...
					String prev_id, prev_title, prev_actor, recommend_title, recommend_prod_id;
									...
									do {
										...
										prevproddetailResult = prevproddetail_queryStatement.executeQuery(
												"select TITLE, ACTOR from DS2.PRODUCTS where PROD_ID=" + prev_id + ";");
										...
										relatedprodResult = relatedprod_queryStatement.executeQuery(
												"select TITLE from DS2.PRODUCTS where PROD_ID= (select COMMON_PROD_ID from DS2.PRODUCTS where PROD_ID="
														+ prev_id + ");");
 ...
 %>
		<FORM ACTION="./dsbrowse.jsp" METHOD=GET>
			...
			<%
						...
						{
			%>
			<H2>Username/password incorrect. Please re-enter your username
				and password</H2>
			<FORM ACTION="./dslogin.jsp" METHOD=GET>
				Username <INPUT TYPE=TEXT NAME="username" VALUE='<%=username%>'
					SIZE=16 MAXLENGTH=24> Password <INPUT TYPE=PASSWORD
					NAME="password" SIZE=16 MAXLENGTH=24> <INPUT TYPE=SUBMIT
					VALUE="Login">
			</FORM>
